...
@Component({
  selector: 'app-cn-time-axis-chart',
  templateUrl: './cn-time-axis-chart.component.html',
  styles: [`
  .g2-label-item {
        font-size: 10px;
        text-align: center;
...
export class CnTimeAxisChartComponent extends CnComponentBase
...
public ngOnInit() {
  ...
  this.y = this.config.chartConfig.BasiAttribute.y.name

  if (this.config.chartConfig.BasiAttribute.y1) {
    this.y1 = this.config.chartConfig.BasiAttribute.y1.name
  }
  ...
  if (this.config.chartConfig.guideConfig) {
    ...
    this.color = this.config.chartConfig.guideBasicAttribute.color ? this.config.chartConfig.guideBasicAttribute.color : '#FF0000'

    this.maxText = this.config.chartConfig.guideBasicAttribute.maxtext ? this.config.chartConfig.guideBasicAttribute.maxtext : '上限'

    this.minText = this.config.chartConfig.guideBasicAttribute.mintext ? this.config.chartConfig.guideBasicAttribute.mintext : '下限'
  ...
  if (this.config.chartConfig.BasiAttribute.groupName) {
    this.groupName = this.config.chartConfig.BasiAttribute.groupName
  }
...
public generateCharts() {
  const guideCfgId = this.config.chartConfig.BasiAttribute.overid ? this.config.chartConfig.BasiAttribute.overid : ''
  let guideCfg
  if (this.config.chartConfig.showUserPoint) {
    if (this.config.chartConfig.BasiAttribute.groupName) {
      this.chart
      ...
      if (this.config.chartConfig.overConfig) {
        guideCfg = this.config.chartConfig.overConfig.find(e => e.id === guideCfgId);
        ...
        if (guideCfg) {
          ...
          if (this.y1) {
            if (guideCfg.y1Rule) {
              this.chart.point().position(this.x + '*' + this.y1)
                .size(4)
                .shape('overDataY1Rule')
                .style({
                  stroke: '#fff',
                  lineWidth: 1
                });;  // 创建柱图特殊写法  X*Y  'caseName*caseCount' year*sales
            ...
            this.chart.line().position(this.x + '*' + this.y1).shape('circle').color('#FFCC00');
            this.chart.point().position(this.x + '*' + this.y1).color('#FFCC00').size(4).shape('overY1').style({
              stroke: '#fff',
              lineWidth: 1
            });
          }
        ...
        this.chart.line().position(this.x + '*' + this.y1).shape('circle').color('#FFCC00');
        this.chart.point().position(this.x + '*' + this.y1).size(4).shape('overY1').style({
          stroke: '#fff',
          lineWidth: 1
        });
        if (this.y1) {
    ...
    if (this.config.chartConfig.BasiAttribute.groupName) {
      ...
      if (this.y1) {
        this.chart.line().position(this.x + '*' + this.y1).shape('circle').color('#FFCC00');
        this.chart.point().position(this.x + '*' + this.y1).color('#FFCC00').size(4).shape('circle').style({
          stroke: '#fff',
          lineWidth: 1
        });
      }
...
public registerPointStyle() {
  ...
  const guideCfgId = this.config.chartConfig.BasiAttribute.overid ? this.config.chartConfig.BasiAttribute.overid : ''
  // 在Y轴的范围之外的提示
  Shape.registerShape('point', 'overY', {
    drawShape(cfg, container) {
      ...
      if (that.guidedataList.length > 0) {
        that.guidedataList.forEach(e => {
          if (that.transStringTime(data[that.x]) <= that.transStringTime(e[that.end])
            && that.transStringTime(data[that.x]) >= that.transStringTime(e[that.start])
            && e[that.guideFiled] === that.y) {
            if (e[that.maxValue] && e[that.minValue]) {
              if (!(data[that.y] <= e[that.maxValue] && data[that.y] >= e[that.minValue])) {
                condition = 1
              }
            } else if (e[that.maxValue]) {
              if (data[that.y] <= e[that.maxValue]) {
                condition = 1
              }
            } else if (e[that.minValue]) {
              if (data[that.y] >= e[that.minValue]) {
                condition = 1
              }
            }
          }
      ...
      if (condition === 1) {
        if (guideCfgId !== '') {
          guideCfg = that.config.chartConfig.overConfig.find(e => e.id === guideCfgId)
        }
        color = guideCfg.y
  ...
  // 在Y1轴(右侧Y轴)的范围之外的提示
  Shape.registerShape('point', 'overY1', {
    drawShape(cfg, container) {
      const data = cfg.origin._origin;
      const point = { x: cfg.x, y: cfg.y };
      let condition = 0; // 判断规则值数组和数据源的条件关系
      if (that.guidedataList.length > 0) {
        that.guidedataList.forEach(e => {
          if (that.transStringTime(data[that.x]) <= that.transStringTime(e[that.end])
            && that.transStringTime(data[that.x]) >= that.transStringTime(e[that.start])
            && e[that.guideFiled] === that.y1) {
            if (e[that.maxValue] && e[that.minValue]) {
              if (!(data[that.y1] <= e[that.maxValue] && data[that.y1] >= e[that.minValue])) {
                condition = 1
              }
            } else if (e[that.maxValue]) {
              if (data[that.y1] <= e[that.maxValue]) {
                condition = 1
              }
            } else if (e[that.minValue]) {
              if (data[that.y1] >= e[that.minValue]) {
                condition = 1
              }
            }
          }
    ...
    drawShape(cfg, container) {
      ...
      if (that.ruledataList.length > 0) {
        that.ruledataList.forEach(e => {
          if (data['STAGE'] === e['STAGE'] && data['NUMBER'] === e['NUMBER'] && !data[e['FILED']]) {
            condition = 1;
          } else if (data['STAGE'] && data['NUMBER'] && !that.ruledataList.find(e => e['NUMBER'] === data['NUMBER'])) {
            condition = 1;
          }
        })
      }
...
public axisInit() {
  ...
  if (this.config.chartConfig.BasiAttribute.y.scale) {
    this.chart.scale(this.y, this.config.chartConfig.BasiAttribute.y.scale);
  }
  if (this.config.chartConfig.BasiAttribute.y.axis) {
    this.chart.axis(this.y, this.config.chartConfig.BasiAttribute.y.axis);
    if (this.config.chartConfig.BasiAttribute.formatConfig && this.config.chartConfig.BasiAttribute.formatConfig.y) {
      this.axisFormatter(this.y, this.config.chartConfig.BasiAttribute.y.axis);
    }
  }
  if (this.y1) {
    if (this.config.chartConfig.BasiAttribute.y1.scale) {
      this.chart.scale(this.y1, this.config.chartConfig.BasiAttribute.y1.scale);
    }
    if (this.config.chartConfig.BasiAttribute.y1.axis) {
      this.chart.axis(this.y1, this.config.chartConfig.BasiAttribute.y1.axis);
      if (this.config.chartConfig.BasiAttribute.formatConfig && this.config.chartConfig.BasiAttribute.formatConfig.y1) {
        this.axisFormatter(this.y1, this.config.chartConfig.BasiAttribute.y1.axis);
      }
    }
  }
}
