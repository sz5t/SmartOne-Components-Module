import { Component, OnInit, AfterViewInit, Input, Inject, ViewChild, ElementRef, EventEmitter } from '@angular/core';
...
@Component({
  ...
  styleUrls: ['./cn-grid-charts.component.less']
})
 ...
 */
  ...
  */
  ...
  public gridSingleBarChart() {
    ...
    chart.interval().position('1*' + field).opacity(1).size(20).label([field], {
      offset: 10,
      textStyle: {
        fontSize: 12,
        color: '#595959'
      ...
      formatter: (val) => {
        return beforeText + val + afterText;
      }
    })
  ...
  public gridBrokenLineChart(data) {
    ...
    chart.area().position(this.x + '*' + this.y).shape('smooth').opacity(0.2);
    chart.line().position(this.x + '*' + this.y).opacity(1).shape('smooth');
    // chart.guide().html({
    //   position: ['120%', '0%'],
  ...
  public gridBarChart(data) {
    ...
    chart.interval().position(this.x + '*' + this.y).opacity(1);
    chart.render();
  }
  ...
  public gridRatioBarChart(data) {
    ...
    chart.axis(false);
    chart.coord().transpose();
    chart.intervalStack().position(this.x + '*' + this.y).color(this.type).size(30).opacity(1);
    chart.render();
  }
}
